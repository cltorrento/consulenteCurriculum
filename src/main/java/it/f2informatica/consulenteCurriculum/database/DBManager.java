package it.f2informatica.consulenteCurriculum.database;
...
public class DBManager {
	...
	@Autowired
	public DBManager(
			@Value("${couchbase.host}") String host, 
			@Value("${couchbase.bucket}") String bucketName, 
			@Value("${couchbase.user}") String user,
	...
	public static Bucket getBucket() {
		if (bucket == null || bucket.isClosed()) {
			logger.debug(
					"Configurazione: host " + host + ", bucket " + bucketName + 
